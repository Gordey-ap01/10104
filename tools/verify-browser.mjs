...
const userDataDir = path.join(os.tmpdir(), `service-10104-chrome-${Date.now()}`);
const failures = [];
const consoleErrors = [];
...
try {
  ...
  await expect(cdp, "10104 primary hero CTA", "document.querySelector('.hero__actions .btn-primary')?.textContent.trim() === 'Выбрать ремонт'");
  await expect(cdp, "10104 description moved", "document.querySelector('.hero__lead') === null && document.querySelector('.hero-story__text') !== null");
  await expect(cdp, "revival counter present", "document.querySelector('[data-revival-counter]') !== null");
  await expect(cdp, "10104 counter enlarged", "document.querySelector('.revival-counter__number').getBoundingClientRect().height >= 72");
  await expect(cdp, "10104 gadget panel opacity", "Math.abs(parseFloat(getComputedStyle(document.querySelector('.hero-story .hero-visual')).opacity) - 0.777) < 0.02");
  ...
  await expect(cdp, "10104 mobile gadget panel visible", "getComputedStyle(document.querySelector('.hero-story .hero-visual')).display !== 'none'");
